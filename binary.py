        ...
        if mousePressed():
            ...
            if not revealed[num]:
                ...
                if wordlist[num]== target:
                    ...
                    moveLabel(targetLabel, 750, 950)
                    return
                elif wordlist[num] < target:
                    for i in range(0,num+1):
                        reveal(i,labels, wordlist, "grey")
                    changeLabel(targetLabel, wordlist[num] + " is lower than " + target + ". Removing lower section")
                    moveLabel(targetLabel, 750, 950)
                else:
                    for i in range(num,100):
                        reveal(i,labels, wordlist, "grey")
                    changeLabel(targetLabel, wordlist[num] + " is higher than " + target + ". Removing upper section")
                    moveLabel(targetLabel, 750, 950)
        updateDisplay()
        tick(100)
